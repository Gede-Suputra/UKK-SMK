<x-layouts.app>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <form method="GET" action="" class="flex items-center gap-3 w-full md:w-auto">
            ...
            <select name="kategori" onchange="this.form.submit()" class="px-4 py-2 text-sm rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-200 focus:outline-none">
                <option value="">Semua Kategori</option>
                @foreach(($kategoris ?? []) as $kat)
                    <option value="{{ $kat->id }}" {{ request()->input('kategori') == $kat->id ? 'selected' : '' }}>{{ $kat->nama_kategori }}</option>
                @endforeach
            </select>
                    ...
                    <tr class="hover:bg-zinc-50/80 dark:hover:bg-zinc-800/30 transition-colors group">
                        ...
                        <td class="px-4 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-1.5">
                                <a href="#" data-modal-open data-modal-target="#alat-modal" data-url="{{ route('alats.show', $a) }}" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-700 text-white text-[11.5px] font-medium transition-colors">Detail</a>
                                <a href="#" data-modal-open data-modal-target="#alat-modal" data-url="{{ route('alats.edit', $a) }}" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-[11.5px] font-medium transition-colors">Edit</a>
